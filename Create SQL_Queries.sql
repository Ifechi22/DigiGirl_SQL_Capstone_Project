...
SELECT months, years, SUM(profit) AS least_profit_generated 
FROM international_breweries
...
SELECT years, brands AS beer, region, SUM(quantity) AS amt_consumed SUM(profit) AS total_profit 
FROM international_breweries
WHERE countries = 'Nigeria' AND brands NOT LIKE '%malt'
GROUP BY years,brands, region
ORDER BY years, amt_consumed;
